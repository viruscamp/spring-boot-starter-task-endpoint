...
public class ScheduledTaskEndpoint
	...
	private List<ScheduledTaskInformation> list = new ArrayList<ScheduledTaskInformation>();
	...
	private List<ScheduledTaskInformation> build(final List<? extends Task> tasks, final ScheduledType type) {
		List<ScheduledTaskInformation> results = new ArrayList<ScheduledTaskInformation>(tasks.size());
		for (Task task : tasks) {
			...
			results.add(scheduledTaskInformation);
		}
		return results;
	}
